...
server <- shinyServer(function(input, output, session){
  ...
  output$timePlot <- renderPlotly({
    ...
    # if(box2() == "nattacks"){
    #   data1 <- completeFun(yearsFilteredData(), c(box1()))
    #   data1 <- data1 %>% group_by(iyear, data1[,which(colnames(data1) == box1())]) %>% tally(sort = TRUE)
    #   colnames(data1) <- c("Year", input$selBox1, input$selBox2)
    # }
    # 
    # else{
    #   data1 <- completeFun(yearsFilteredData(), c(box2(), box1()))
    #   data1 <- aggregate(data1[,which(colnames(data1) == box2())], by=list(year = data1$iyear, c = data1[,which(colnames(data1) == box1())]), FUN=sum)
    #   colnames(data1) <- c("Year", input$selBox1, input$selBox2)
    # }
    # 
    # ggplot(data1, aes(x = unlist(data1[,1]), y = sqrt(unlist(data1[,3])), colour = unlist(data1[,2]), group = interaction(unlist(data1[,2])))) +
    #   geom_point(size=3) +
    #   geom_line(size=1) +
    #   labs(x = colnames(data1)[1], y = paste("SQRT of ", colnames(data1)[3]), colour = colnames(data1)[2])
  })
